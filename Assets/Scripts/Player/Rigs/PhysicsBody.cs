...
namespace UVD.Player
{
    public class PhysicsBody : MonoBehaviour
    {
        ...
        public PhysicsHand rightHand, leftHand;
        ...
        private void FixedUpdate()
        {
            ...
            // @Incomplete:
            // Apply inverse hand forces to the body (make sure masses prevent player from swinging themselves around)
            bodyCore.AddForce(-rightHand.AppliedForce);
            bodyCore.AddForce(-leftHand.AppliedForce);

            // Add body movement to the hands to prevent lagging behind
            rightHand.BodyVelocityCompensation(bodyCore.velocity);
            leftHand.BodyVelocityCompensation(bodyCore.velocity);

            // InputRig lag compensation
            inputRig.vrRoot.position += bodyCore.velocity * Time.fixedDeltaTime;
